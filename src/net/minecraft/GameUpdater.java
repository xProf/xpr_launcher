...
public class GameUpdater implements Runnable
{
    ...
    if( path.exists() ) 
        ...
        for(int i=0; i<files.length; i++) 
        {
            if(files[i].isDirectory()) 
                {
                    if ((!files[i].getName().equalsIgnoreCase("Saves"))&& (!files[i].getName().equalsIgnoreCase("Stats"))&&(!files[i].getName().equalsIgnoreCase("texturepacks"))&&(!files[i].getName().equalsIgnoreCase("screenshots"))) deleteDirectory(files[i]);
                }
                else 
                    {
                    if ((!files[i].getName().equalsIgnoreCase("minecraft.exe"))&&(!files[i].getName().equalsIgnoreCase("lastlogin"))&&(!files[i].getName().equalsIgnoreCase("options.txt"))) files[i].delete();
                    }
        }
        }
    return( path.delete() );
  ...
  protected String getDescriptionForState()
  {
    switch (state) {
    case 1:
      return "Ініціалізація завантаження";
    case 2:
      return "Пошук пакетів для завантаження";
    case 3:
      return "Перевірка кеш-файлів";
    case 4:
      return "Завантаження пакетів";
    case 5:
      return "Розархівування завантаженних пакетів";
    case 6:
      return "Оновлення шляхів";
    case 7:
      return "Звертання аплету";
    case 8:
      return "Ініціалізація реального аплету";
    case 9:
      return "Старт реального аплету";
    case 10:
      return "Завантаження завершено";
    }
    return "Невідоме доповнення";
  }
  ...
  protected void loadJarURLs() throws Exception {
    ...
    if (osName.startsWith("Win"))
      ...
      fatalErrorOccured("OS (" + osName + ") не підтримується", null);
    }

    if (nativeJar == null) {
      fatalErrorOccured("lwjgl файли не знайдені", null);
    } else {
